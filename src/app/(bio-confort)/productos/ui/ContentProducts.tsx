      ...
      <div className="bg-white rounded-lg shadow-lg w-3/4 p-6 relative" onClick={(e) => e.stopPropagation()}>
        <div className='flex items-center justify-center'>
          ...
          <div className='w-1/2 text-sm'>
            <h2 className="text-xl font-bold mb-8 text-center ">{product.title}</h2>
            <h3 className='font-bold mb-3 text-base'>{product.subtitle}</h3>
            ...
            <ul className='list-decimal ml-4'>
              {product.features.map((feature, index) => (
                <li key={index} className='pt-4'>{feature}</li>
              ))}
            </ul>
            ...
            <ul className='list-decimal ml-4'>
              {product.benefits.map((benefit, index) => (
                <li key={index} className='pt-4'>{benefit}</li>
              ))}
            </ul>
      ...
      <div className="py-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 place-items-center">
        {products.map((product, index) => (
          <div key={index} className="flex flex-col items-center">
            <div className="bg-[#F4F4F4] w-80 rounded-2xl p-4 flex flex-col">
              <Image
                src={product.imageSrc}
                alt={product.title}
                width={300}
